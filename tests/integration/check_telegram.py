...
class FakeAPI:
    ...
    def send(self, chat_id, text, markup=None, silent=False, thread_id=None):
        self.sent.append({"chat": chat_id, "text": text, "markup": markup, "silent": silent}); return [{}]
...
def main():
    conn = M.connect()
    cleanup(conn)
    notify_before = conn.execute("SELECT value FROM control WHERE key='telegram:notify'").fetchone()
    try:
        ...
        cmds = ["/start", "/help", "/status", "/status itest-tg", "/wallets", "/balance", "/count", "/signals", "/prices", "/profit", "/profit itest-tg",
                ...
                "/forceexit itest-tg", "/gibtsnicht", "/profit xyz", "/status@tradebot_bot"]
        bad = []
        for c in cmds:
            try:
        ...
        if notify_before:
            conn.execute("UPDATE control SET value=%s WHERE key='telegram:notify'", (notify_before["value"],))
        else:
            conn.execute("DELETE FROM control WHERE key='telegram:notify'")
        conn.commit()
        cleanup(conn)
